...
namespace Cashbaazi.Game.FruitNinja
{
    ...
    public class FNCTouchSlicer : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            //will get our direction for fruit cuts/particle splatters.
        ...
        private void GrabSlicingDevice()
        {
            ...
            if (useColliderAndRaycast)
            {
                ...
                //***obso
                //sliceColliderParent.SetActive(true);      //set active  ****Was still activating before move finished****  Put this in a one line method so we could invoke it late.
                //***obso

                //since the ray-cast does the bulk of the work anyway, its okay that the collider is not on instantly

                //call/invoke the collider activation with half second delay.  Even though we were moving it first and THEN activating it... it was STILL being activated in the old position, or
                //at the very least IN TRANSIT... silliness.  So we have to make a stupid one line method so we can call it in 1/4 to 1/2 second later... sorry guys.
        ...
        /// <summary>
        /// This Method is responsible for releasing the FNC_TouchSlicer GO, and Deactivating the "Slice" collider we have
        /// protruding into the scene.  This is called when the user stops touching or holding mouse1 depending on settings.
        /// The collider is not often responsible for destroying fruit, usually it is the Ray-cast, but i wanted some 
        ...
        /// and come at a fruit from a fixed location a couple of fruit lengths away, then the cuts are pretty accurate.  As accurate as the 8 direction cut can be. Up,
        ...
        /// with several fruit), then I would love to see it!!  After spending way to long on this I settled because it was super simple/understandable... forgive me!! lol
        /// </summary>
        /// <remarks>
        ...
        /// actual mesh deformations, filled faces, and displayed the "fruit Core" UV coordinates.  Then another version that at the very least had accurate cut orientations, 
        ...
        /// It checks the angle of the swipe and based on the direction it spawns the correct fruit debris prefabs.  (i.e. if you swipe up/down it
        /// will spawn left and right Fruit Halves.  If you swipe left/right it will spawn top and bottom Fruit Halves).  If you swipe up-left
        ...
        /// because some of the gameModes are more difficult than others... so there is a Arcade,Classic, and RelaxModeScore.
        /// </summary>
        private void AddToFruitDestroyedScore()
        {
            //if the GameControllers "gameModes" var is set to GameModes.Arcade then...
            if (GameController.GameControllerInstance.gameModes == GameModes.Arcade)
            {
                //then ArcadeModeScore gets incremented by 1 :-)
                GameVariables.ArcadeModeScore++;
            }
